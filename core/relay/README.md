...
The Ployz-hosted [iroh relay](https://github.com/n0-computer/iroh/tree/main/iroh-relay) that
the management transport uses when a Machine sits behind NAT. It runs as one Compose service
behind Caddy at `https://relay.ployz.dev`, the compiled default relay URL in the core crate.
Access is open; the relay's per-client rate limits in `iroh-relay.toml` are the abuse control. Add an allowlist only if abuse appears.
...
Deploy `compose.yaml` to the host; its `x-ports` entry publishes port 80 through Caddy,
