...
#[test]
fn automatic_sites_render_routes_and_health_endpoint() {
...
fn shared_renderer_projection_drives_caddy() {
    ...
    assert!(pin.contains(".crt") && pin.contains(".key"), "{pin}");
}
...
fn automatic_sites_keep_unreachable_hosts_and_omit_unassigned_ports() {
    ...
    assert!(caddyfile.contains(INGRESS_VERIFY_PATH));
    assert!(caddyfile.contains("http://missing.example"));
    assert!(caddyfile.contains("respond \"Bad Gateway\" 502"));
...
async fn custom_configs_use_latest_specs_render_upstreams_and_isolate_failures() {
    ...
    assert!(caddyfile.contains("Service 'app/invalid': validation failed"));
