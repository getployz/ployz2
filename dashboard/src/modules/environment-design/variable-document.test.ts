...
const intent: SavedEnvironmentIntent = { version: 1, environmentSlug: "production", services: [], variableGroups: [], volumes: [] };
const variable: VariableRecord = {
  id: "00000000-0000-4000-8000-000000000001", serviceId: "00000000-0000-4000-8000-000000000002", variableGroupId: null,
...
it("saves an unknown reference as text and projects a warning after reload", async () => {
  ...
  const loaded = variableDocumentRecord(saved, variable, intent, new Date(0));
  ...
  expect(variableDocumentRecord(corrected, variable, intent, new Date(0)).unresolvedReferences).toEqual([]);
});
...
it("does not inspect sealed values", () => {
  ...
  expect(variableDocumentRecord(saved, variable, intent, new Date(0)).unresolvedReferences).toEqual([]);
});
