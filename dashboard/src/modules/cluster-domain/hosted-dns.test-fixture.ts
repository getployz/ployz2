...
/** An in-process Hosted DNS: grants `<preferred>.ployz.test`, answers every other call with 204, or fails every call with `failWith`. */
export async function startFakeHostedDns() {
  ...
  const server = createServer((request, response) => {
    ...
    request.on("end", () => {
      ...
      } else if (request.method === "POST" && request.url === "/domains") {
