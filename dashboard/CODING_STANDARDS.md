...
5. ✓ **Remote Reads declare freshness.** Every `queryOptions`, and every options object with a `queryFn`, sets `staleTime`; only Org Store tables in `src/collections/` inherit the `createApiCollection` default.
...
8. ✓ **Loaders preload the page.** A page's loader prefetches every Remote Read the page shows (`prefetchRemote`), so navigation lands on data already in memory. Reads that depend on a user action (a picked repository, a search, opening logs) are listed as on demand in the boundary test with what warms them instead.
...
10. ✓ **Environment documents change through `editEnvironmentDocument`.** It applies the edit, queues saves per environment against the latest revision (the server rejects stale ones), and owns the failure toast. Only listed commands that receive a whole new document write it directly.
