...
#[tokio::test]
async fn zentinel_global_does_not_require_a_bridge_endpoint() {
    let requested = ployz::ingress::service_spec_for_backend(
        ployz_core::IngressProxyBackend::Zentinel,
        Some("example.test/zentinel:new".into()),
        Vec::new(),
        None,
    )
    .await
    ...
    assert!(matches!(
        operations(&plan).as_slice(),
        [DeployOperation::RunContainer { machine_id: actual, .. }]
            if actual == &machine_id('1')
    ));
}

#[tokio::test]
async fn zentinel_replacement_stops_the_old_host_network_proxy_first() {
    let requested = ployz::ingress::service_spec_for_backend(
        ployz_core::IngressProxyBackend::Zentinel,
        Some("example.test/zentinel:new".into()),
        Vec::new(),
    ...
    assert!(matches!(
        ...
        [DeployOperation::ReplaceContainer(ReplacementOperation { spec, .. })]
            if spec.update.order == ployz_core::UpdateOrder::StopFirst
                && spec.container.image == "example.test/zentinel:new"
    ));
}
