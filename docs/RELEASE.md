...
Protected `main` pushes warm the shared R2 cache, including release archives and SDK bindings on relevant changes. These warming runs do not publish releases or npm packages. Tag, release, scheduled, and manual runs read the remote cache; kache only uploads from protected-branch pushes. GitHub Actions variables `KACHE_S3_BUCKET`, `KACHE_S3_ENDPOINT`, and `KACHE_S3_REGION` select the bucket; secrets `KACHE_S3_ACCESS_KEY_ID` and `KACHE_S3_SECRET_ACCESS_KEY` provide access. Fork PRs without credentials fall back to GitHub caching.
