...
- `layer1.tsv` maps the exact 85 top-level semantic oracle groups in the approved Layer 1 families to Rust tests. The count is derived from the pinned source inventory: 33 Compose, 21 Deploy, 12 API/domain, 13 client Caddy/DNS/log-merging, three config/connection, and three binding Caddyfile groups. The dropped Caddy JSON generator is excluded by [#24](https://github.com/getployz/ployz2/issues/24).
